...
let disas_cache : ((string list, dis_error) Either.t) DisCache.t = DisCache.create ~max_size:10000 ()
...
let to_asli (opcode_be: string) (addr : int) : ((string list, dis_error) Either.t) =
  if cache then (
