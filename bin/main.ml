...
type opcode_sem = ((string list, dis_error) Either.t)
...
module InsnLifter = struct 
  ...
  let disas_cache : ((string list, dis_error) Either.t) DisCache.t = DisCache.create ~max_size:5000 ()
  ...
  let to_asli_impl (opcode_be: string) (addr : int) : ((string list, dis_error) Either.t) =
  ...
  let do_dis () : ((string list * string list), dis_error) Either.t =
    ...
    | res -> 
        ...
        Left (List.map p_raw res, List.map p_pretty res)
    | exception exc ->
  ...
  let to_asli ?(cache=true) (opcode_be: string) (addr : int) : ((string list, dis_error) Either.t) =
    if cache then (
